...
module.exports = class ProgressBar {
	constructor() {
		this.total;
		this.current;
		this.bar_length = process.stdout.columns - 60;
	}
	...
	update(current, num, total) {
		this.current = current;
		const current_progress = this.current / this.total;
		this.draw(current_progress, num, total);
	}

	draw(current_progress, num, total) {
		...
		process.stdout.write(
			`Writing files (${num}/${total})... [${filled_bar}${empty_bar}] | ${percentage_progress}%`
		);
	}
